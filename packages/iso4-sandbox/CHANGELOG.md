...
- e4face8: feat: a 128 MB host reserve, reconstructible capacity refusals, and a startup capacity log
...
- 2885a61: feat: three run-time numbers — `durationMs`, new `wallTimeMs`, `cpuTimeMs` — and a wall limit that counts only the run's own time (#148)
...
- 2513005: feat: sandbox response bodies stream back to the host (#128)
...
- f9e3e3c: feat: soft and hard aborts — `signal` becomes a boundary abandon, new `hardAbortSignal` escalation (#75)
...
- 96ee6f4: feat: dedicated error codes for reserved-name shadowing and invalid rebinds (#162)
...
- 1819c12: feat: retire a grown warm instance instead of killing a run (#169)
...
- 4870938: fix: the sandbox console keeps V8's full method set (#86)
...
- 4870938: fix: `console.log` renders objects instead of `[object Object]`
...
- bf26660: fix: streamed request bodies arrived truncated when concurrent streaming runs shared a warm instance (#170)
...
- 3b4c07c: feat: remove WebAssembly from the sandbox
