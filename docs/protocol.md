...
| `wallTimeMs`         | `Optional<u32>` | `30000`  | Caps the run's own logic time (the result's `wallTimeMs`): execution + async waits, each counted to its answer's arrival. Zero = no limit.                       |
...
location against the declared shape and fails the run for anything that is
not a declared host-module function leaf: `ERR_UNDECLARED_BINDING` for an
undeclared specifier or path (the same enforcement point that guards
undeclared globals), `ERR_FROZEN_BINDING` for a declared location that is
frozen with the prefix (data leaf, source module).
...
| `slotAllowance` | `u32`                         | Runs the runtime is currently granting the host, derived from the shape of the work it is serving. `0` = no opinion yet. The host admits while its in-flight count is below this and queues the rest; an explicit `maxConcurrentRuns` ignores it. Advisory: the runtime does not enforce it. |
...
| `durationMs`      | `f64`                    | Complete wall-clock, dispatch → processed conclusion; engine time for co-resident runs counts too. May exceed the `wallTimeMs` limit, which caps `wallTimeMs` below instead (DESIGN.md §4.1).                                               |
| `wallTimeMs`      | `f64`                    | The run's own logic time: execution + async waits, each wait counted until its answer arrived; engine time excluded. `cpuTimeMs ≤ wallTimeMs ≤ durationMs`.                                                                                 |
| `cpuTimeMs`       | `f64`                    | CPU burned on the run's own thread; bridge waits excluded, as is time descheduled while co-resident runs hold the cores.                                                                                                                    |
...
| `durationMs`  | `f64`                                   | Complete grace wall time (after the Result shipped).                                                              |
| `wallTimeMs`  | `f64`                                   | The grace phase's own logic time — same rule as the run's `wallTimeMs`.                                           |
| `cpuTimeMs`   | `f64`                                   | CPU burned during grace — metered separately from the run's.                                                      |
...
| `ERR_INVALID_REBIND`                  | **Host-detected, never sent by the runtime.** A rebind offered a value of the wrong type — a function leaf can only be rebound with a function.                                                                                                                                                                                                                                                                                                                             |
